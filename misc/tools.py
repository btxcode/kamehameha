...
class BugHunterPro:
    ...
    def run_whois(self):
        ...
        try:
            # Coba query dulu jika .whois gagal
            if hasattr(whois, 'whois'):
                w = whois.whois(self.host)
            else:
                w = whois.query(self.host)
            
            # Cek jika respon kosong
            if not w:
                raise Exception("Empty response from WHOIS server")

            # Helper aman untuk mengambil atribut
            ...
            registrar = get_attr(w, 'registrar')
            creation_date = get_attr(w, 'creation_date')
            expiry_date = get_attr(w, 'expiry_date')
            name_servers = get_attr(w, 'name_servers')
            org = get_attr(w, 'org')
            country = get_attr(w, 'country')
            ...
            self.log("success", f"WHOIS selesai: {registrar}")
